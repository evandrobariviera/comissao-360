...
-- Parte 2 — produtos com bônus por unidade (ex.: "cada caixa do produto X = R$ 5,00").
-- Catálogo global reutilizável; a participação (R$/unidade + grupo vinculado) é por edição.
--  - Produto COM grupo: o `valor` vendido soma ao valor da grade daquele grupo (conta pro ranking).
--  - Produto SOLTO (grupo_id NULL): não mexe em ranking; só rende o bônus por unidade.
--  - Bônus por unidade = quantidade x bonus_unidade, pago a TODO funcionário que vendeu
--    (independe de posição), tanto pros vinculados quanto pros soltos.
CREATE TABLE corrida_produto (
  id             INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  nome           VARCHAR(120) NOT NULL,
  unidade_rotulo VARCHAR(20)  NOT NULL DEFAULT 'unidade',  -- "caixa", "frasco"...
  ativo          TINYINT(1)   NOT NULL DEFAULT 1,
  created_at     DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE corrida_edicao_produto (
  ...
  grupo_id       INT UNSIGNED NULL,                -- NULL = produto "solto"
  bonus_unidade  DECIMAL(10,2) NOT NULL DEFAULT 0, -- R$ por unidade nesta edição (editável a qualquer momento)
...
-- Grade funcionário x produto participante (acumulado, sobrescrito a cada salvamento).
-- `valor` é obrigatório na tela quando o produto tem grupo (precisa somar ao ranking).
...
-- Snapshot do bônus por unidade no fechamento (total por funcionário).
